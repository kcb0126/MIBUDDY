...
public class EnglishAnswer implements Parcelable {
    ...
    public EnglishAnswer() {};

    public boolean readAnswer(Context context, String ID){
        ...
        cursor = db.query(
                DataBaseContract.DataBaseEntry.TABLE_NAME_ENGLISH_ANSWER, // tabla
                projection, // columnas
                selection, // where
    ...
    protected EnglishAnswer(Parcel in) {
        this.DBID = in.readString();
        this.answerText = in.readString();
        this.audio = in.readInt();
    }

    public static final Creator<EnglishAnswer> CREATOR = new Creator<EnglishAnswer>() {
        @Override
        public EnglishAnswer createFromParcel(Parcel source) {
            return new EnglishAnswer(source);
        }

        @Override
        public EnglishAnswer[] newArray(int size) {
            return new EnglishAnswer[size];
        }
    };
}